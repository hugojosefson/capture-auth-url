 ...
 * - Captures the incoming request URL, and responds with an HTML page that has client-side JavaScript in it that picks up [`window.location.hash`](https://developer.mozilla.org/en-US/docs/Web/API/Location/hash), and sends it in a request into the server too.
 ...
 * @param returnInstructions {string|Response} Instructions to display to the user after the hash is captured. Can be a string or a `Response` object. Defaults to a simple HTML message.
